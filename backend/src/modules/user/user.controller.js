...
import { encrypt, hashValue, wrapKey, generateKey } from '../../common/utils/encryption.js';
...
export async function createUser(req, res) {
  try {
    const { email, phone, role = 'user', organization_token_id } = req.body;
...
export async function getUser(req, res) {
  try {
    ...
    const user = await pg('users')
      ...
      .select('user_token_id', 'role', 'created_at')
      .first();
    ...
    // Cache for 60s
    await redis.setex(`user_cache:${token}`, 60, JSON.stringify(user));
